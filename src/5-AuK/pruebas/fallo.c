...
/* Solución 0: Sin algoritmo
 * Con esto se pretende mostrar cómo se quedan trabados los filósofos y
 * mueren de hambre.
 */
...
int cogerTenedores(int id) {
    ...
    ten1 = id;
    ten2 = (id + 1) % NUM_FIL;
    r1 = lockMutex(&tenedores[ten1]);
    if (r1 != 0) {
        print_t("\t\t\t[%d] MUERE DE HAMBRE [T: %d].\n", id, ten1);
        filosofos[id] = MUERTO;
        return -1;
    }
    r2 = lockMutex(&tenedores[ten2]);
    if (r2 != 0) {
